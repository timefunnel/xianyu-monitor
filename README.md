...
2. **无头浏览器会被识别**。`headless: true` 时首页直接返回「非法访问 为了保障您的体验，请使用正常浏览器访问闲鱼」，搜索请求根本不会发出。因此配置默认 `headless: false`，**服务器上必须用 `xvfb-run`** 提供虚拟显示（这不是伪造指纹，只是让浏览器以正常的有头模式运行）。
...
工作原理：用 Playwright 打开 `www.goofish.com` 扫码登录，登录态存在浏览器 profile 里。搜索**默认不经过页面**——直接拿这份登录态向 `h5api.m.goofish.com` 发一次 mtop 请求（`search.mode: "http"`，每轮恰好 1 次，理由见「请求次数」一节；只用到网页端公开的 H5 `md5` 签名，不碰 App 端依赖 native 库的 `x-sign`/`x-mini-wua`）。把 `search.mode` 设成 `"browser"` 则退回「驱动搜索页、读取页面自己发出的请求」那套——**不复刻任何签名算法**，代价是冷启动连发 4~6 次请求。接口换参数时只需调整 `src/parse.mjs` 一个文件。
...
服务器没有图形界面，而闲鱼会拦截无头浏览器，因此**所有命令都要跑在 `xvfb-run` 下**（Compose 镜像的 entrypoint 已经自动包了一层）。
...
```jsonc
{
  "browser": {
    ...
    "headless": false,                        // 必须 false，无头会被识别为非法访问
    "attach": false,                          // true=自己 spawn 普通 Chrome 再用 CDP 附加（见「风控」一节）
  ...
  "search": {
    ...
    "timeoutMs": 20000,
    "riskCookies": "omit"                     // omit=不发 sgcookie（默认）；remembered=带但剔掉已知会被拒的值
  },
  "monitor": {
...
`sort` 与 `publishDays` 来自页面顶部「**新发布**」那个下拉，两者互相独立：

| 下拉选项 | 走哪里 | 含义 |
| --- | --- | --- |
| 最新 | 请求体顶层 `sortField=create` + `sortValue=desc` | 按上架时间倒序，新货优先 |
| 1 / 3 / 7 / 14 天内 | `propValueStr.searchFilter` 的 `publishDays:N;` | 只取该时间窗内发布的 |

发布时间窗是**服务端过滤**，比用 `maxAgeMinutes` 按发布时间本地判断可靠得多——PC 搜索响应里基本没有发布时间的结构化字段，`maxAgeMinutes` 因此长期处于「按未知放行」的状态。想只盯新货，用 `publishDays`。

`region` 的取值必须与闲鱼区域面板上的文字一致：预设 `江浙沪` / `珠三角` / `京津冀` / `东三省` / `全国`，或省份名（`上海` / `江苏` / `浙江` …）。

实现方式是在页面里驱动原生筛选，不是伪造请求体：价格与发布时间窗走 `propValueStr.searchFilter`（分号分隔，可叠加，如 `priceRange:500,700;publishDays:3;`）、区域走 `extraFilterValue.extraDivision`、排序走请求体顶层的 `sortField`/`sortValue`，这些请求都带签名，自己拼会失效。区域筛选的界面交互是「点开区域面板 → 选地区 → 点**查看 N 件宝贝**确认」，**最后一步才会真正发起搜索**。
...
页面加载时必然先发一次未筛选的搜索，填价格下限时还会发一次 `priceRange:500,undefined` 的中间态——这两批都是**全国范围**的结果。所以程序会解析每次搜索**实际发出的请求体**，确认里面真的带上了配置的价格区间和地区：

| 区域取值 | 请求体里的位置 |
| --- | --- |
| 预设 `江浙沪` / `珠三角` / `京津冀` / `东三省` | `extraFilterValue.extraDivision` |
| 具体省份或城市 `上海` / `江苏` / `杭州` | `extraFilterValue.divisionList`，形如 `[{"province":"上海"}]` |

> 这两种形态是实测出来的。只看 `extraDivision` 会把省份选择误判成「筛选没生效」，进而退回使用包含未筛选批的全部结果——本项目踩过这个坑，一次推了 31 条和关键词无关的商品。

校验结果的处理：

- **对得上** → 只取最后那次带全部条件的搜索结果；
- **对不上** → **本轮不推送任何商品**，抛出错误让主循环立即告警（这种情况不会自己恢复，不值得等阈值）。
...
| `browser` | 冷启动 4~6，之后 1 | 驱动页面；价格与区域各触发一次搜索，页面加载自带一次，「新发布」每项再加一次 |

改成直连的直接原因就是**请求密度**：浏览器方案下冷启动会在约 5 秒内连发 5 次（页面加载 → 价格下限 → 价格上限 → 区域确认 → 「新发布」），而实测「短时间内连续 3 次搜索」就会返回 `RGV587`。也就是说**每一次冷启动都必然越过那条线**，而它会在会话启动、换关键词/条件、以及每次失败后重新加载时反复发生。直连之后这个连发从根上没有了。
...
**价格和地区都只在服务端过滤，不再配一遍客户端条件。** 理由：原生筛选有请求体级别的校验（见上一节），再加一层客户端条件只会让人以为有两道防线，而实际上它掩盖的正是"筛选压根没生效"这种真问题——这个坑本项目踩过：冷启动时把未筛选批也并进结果，客户端价格和地区条件把越界商品挡掉了，看起来一切正常，直到把客户端条件去掉才暴露出来。
...
| 打开控制台的设备 | 点一行会打开 | 为什么 |
| --- | --- | --- |
| 电脑浏览器 | **在监控自己那个已登录的窗口里**新开标签打开网页 | 桌面浏览器直接开闲鱼网页是未登录的，而登录态在监控的 Chrome profile 里；借它的窗口打开才带登录状态。接口打不开时（监控跑在无头模式、浏览器没起来）自动回退到本地浏览器打开，不让点击落空 |
| 手机浏览器 | App 深链 `fleamarket://item?id=...` | 手机上闲鱼能接管这个 scheme，直接进 App；接口没给深链时回退网页链接 |

「点一下看商品」借监控窗口打开，最多同时保留 5 个这样的标签页，超出的关掉最旧的；各任务的搜索页不受影响，不会被顶掉。

**关于商品分享码 / 二维码**：PC 网页商品页**没有**分享入口（我对全页所有元素的 `title`/`aria-label`/`alt`/`href`/`class` 做了穷举匹配，`分享|二维码|share|qrcode|复制|口令` 全部无命中），分享码是 App 端功能。但**不需要去解码二维码**——搜索接口给每个商品都返回了 App 深链 `targetUrl`（实测 30/30 条都有，且逐商品不同），扫码得到的跳转目标就是同一族地址：
...
- **间隔可以设到 120 秒左右**。稳态每轮只有 1 次请求（热刷新），冷启动 4 次；实测触发风控的节奏是每分钟 3 次量级，留足余量即可。多任务会串行执行，总请求量按 `1/间隔` 累加。
...
- **调参和排查都不要短时间反复试探**。实测连续几次搜索就会触发风控（先弹 baxia 验证层，严重时直接让登录态失效，需要重新扫码）。要验证筛选是否真的生效，用 `node debug-single-cycle.mjs`：它配好条件只跑一轮就停（冷启动 4 次请求），跑完任务保持停用，筛选留在配置里。
- **价格和区域都用 `nativeFilters`**（服务端过滤，30 个名额全落在条件内），不要再配一遍客户端条件。冷启动 4 次请求，之后每轮 1 次，所以 `intervalSeconds` 可以设到 120 秒左右。
...
| 日志出现「原生筛选没有生效」，并附上一行「实际最近几次请求」 | 某一项条件没进请求体。对比那一行里的 `searchFilter` / 区域 / 排序就能看出是哪一项，通常是闲鱼改版导致选择器失配；也可能是风控在交互中途挡住了点击 |
...
| 页面出现「非法访问 为了保障您的体验，请使用正常浏览器访问闲鱼」 | 用了无头模式。把 `browser.headless` 设为 `false`，服务器上用 `xvfb-run` 启动 |
...
闲鱼用的是阿里系的 **baxia 风控**。撞上之后会弹一个验证层盖住页面，所有点击都被它拦截，日志里会看到「闲鱼弹出了风控验证（baxia 弹层）」。

**关键一点：这个验证在监控的窗口里过不了。** 那个窗口受 Playwright/CDP 控制，带自动化特征，baxia 会认出来——实测即使手动操作正确也会被判失败。所以**不要**在监控的窗口里反复试，那只会让风控更严。

正确的处理步骤：

1. **停掉监控进程**（Ctrl+C 或 `docker compose stop`）。它会一直占着浏览器 profile，不释放就打不开。
2. 用**普通 Chrome** 打开同一个 profile：

   ```bash
   node open-profile-in-chrome.mjs
   ```

   它会读出配置里的 `browser.userDataDir`，用系统 Chrome（不带自动化特征）打开闲鱼。也可以手动执行：

   ```bash
   chrome.exe --user-data-dir="<项目目录>/data/browser-profile" https://www.goofish.com/
   ```

3. 在这个普通 Chrome 里完成验证 —— 该过就过了。顺手正常浏览一会儿（翻两页搜索结果）让风控等级降下来。
4. 关掉这个 Chrome 窗口，重新启动监控。

程序侧的配合：撞上风控会**立刻告警**并按 `monitor.riskControlCooldownSeconds`（默认 30 分钟）长时间退避。这个值故意给得很长——验证过不了，重试没有意义，只会让风控等级一直下不来。

**最省事的办法是从源头上不撞它**：`intervalSeconds` 保持在 120 秒以上、不要短时间内反复手动翻搜索页调试（要验证筛选用 `debug-single-cycle.mjs` 跑一轮）、多任务时留意启动时那条总频率提醒。

### 被「访问被拒绝」拦住（RGV587 + action=deny）

同一个 `RGV587_ERROR` 有两种完全不同的形态，处理方式相反，但错误码一模一样，只能看处罚链接里的 `action`：

| 处罚链接里的 `action` | 页面上显示 | 含义 | 怎么办 |
| --- | --- | --- | --- |
| `captcha` / `verify` | 滑块验证 | 要求过验证 | 用普通 Chrome 过掉（见上一节），拿到 `x5sec` 就恢复 |
| `deny` | 「亲，访问被拒绝」+ 二维码 | **直接拒绝，没有可操作的验证项** | 过验证这条路无效，见下 |

日志里看不出区别，所以有一个命令专门做这件事：

```bash
node diagnose-risk.mjs           # 只加载一次搜索页，给出结论和下一步该做什么
node diagnose-risk.mjs --attach  # 换成「普通 Chrome + CDP 附加」再试一次
```

一次导航就能拿到三件日志里没有的事：会话是不是真的有效（`loginuser.get` 的返回码）、**同一批请求里别的 mtop 接口成不成功**（只有 search 失败＝搜索接口被单独风控，全都失败＝会话问题）、处罚页给的是 `deny` 还是验证。撞上 `deny` 时主循环不再每 5 分钟空转，而是按 `riskControlCooldownSeconds` 长时间退避并明确告警——重试对它是没有意义的。

处置顺序（把代价低的放前面）：

1. **什么都别做**。实测这个拒绝由 `sgcookie` 携带，而**默认策略是根本不发它**（`search.riskCookies: "omit"`），所以正常情况下压根不会撞上——不需要"先试一次再自愈"。如果你把它设成 `"remembered"`（带上，但剔掉已知会被拒的具体值），那么每遇到一个**新的**脏值会先失败一次，下一轮按正常间隔重试即可。
2. **自愈也失败时，停手 2~6 小时**。社区共识（`fancyboi999/goofish-cli` 的 `x5sec-recovery.md`）是：RGV587 属服务端判定，本地解不了；**不要重新登录**，重新导 cookie 会覆盖掉还残留 `x5sec` 的会话；正确做法是让会话静静冷却。本项目此前把它误判成限流、每 5 分钟重试一次，只会让分数更低——现在不会了。
3. **仍不行再复位设备标识**：把 `data/browser-profile` 改名备份，重新 `node src/cli.mjs login` 扫码。注意**只把登录 cookie 搬到新 profile 是不够的**——会话认设备，实测搬完直接退化成未登录，必须真的重新扫码。

### 这一节的实测根因（2026-09-24）

触发条件被定位到了一个具体的 cookie，而不是"环境看起来像不像机器人"：

| 请求里带什么 | 结果 |
| --- | --- |
| 全部 cookie | `RGV587` + `action=deny` |
| **只摘掉 `sgcookie`** | **`SUCCESS`，正常返回 30 条商品** |
| 只摘掉 `cbc` | 照样被拒 |
| 只摘掉 `_samesite_flag_` | 照样被拒 |

`sgcookie` 是阿里 SecurityGuard 下发的风控状态令牌（`.goofish.com`，有效期一年）——**它脏了之后，带着它的请求一律被拒；不带它就正常**。

因此默认 `search.riskCookies: "omit"`：**这类 cookie 一律不发**。理由是它并不承担任何功能，只是把平台施加的处罚带过来；而不带它请求照常成功。「先带一次、被拒、再记住」要白白浪费一次注定失败的请求，而且每遇到一个新脏值都要浪费一次——那是最不该付出的代价，因为**被拒的请求本身就是又一次风控压力**。

> 这个选择是明摆着的取舍：它确实等于让客户端不再携带平台用来施加这道处罚的凭据。这是账号所有者自己的判断，所以留成开关。想恢复"带上、但剔掉已知会被拒的值"就设成 `"remembered"`——好处是服务端换发新 sgcookie 时客户端有机会自己回到正常状态。

推论（都被实测证实）：

- **和客户端形态完全无关**。不启浏览器的**纯 HTTP 请求**（只带 cookie + H5 的 `md5(token&t&appKey&data)` 签名）会复现同样的拒绝；去掉自动化标记的 attach 模式也一样。
- **不是账号问题**：同一批请求里另外 8 个 mtop 接口（含 `loginuser.get`）全部 `SUCCESS`，只有搜索接口被拒；手机 App 也正常。
- **不是频率问题**：停机 7.7 小时后**第一个**请求就被拒。

已经实测排除的路，别再花时间：

- **换 UA / 手机版网页**。`www.goofish.com/search` 在手机 UA 下返回的是同一个 PC SPA，接口仍是 `...pc.search`；`m.goofish.com` 不存在，`h5.m.goofish.com` 只有商品分享页。**没有 H5 搜索站**。
- **去掉自动化特征**。`browser.attach: true` 会自己 spawn 普通 Chrome（不加 Playwright 那套开关，页面里 `navigator.webdriver` 为 false）再 CDP 附加。实测对 `deny` **没有任何影响**——原因是 `connectOverCDP` 与 `launch` 在 CDP 命令层走的是同一条 `FrameSession._initialize()`，`Runtime.enable` 照发，所以"自己 spawn"只移掉了浏览器侧的 flag，移不掉 driver 发的信号。这是两条独立的检测面，选项留着，但别指望它救 `deny`。
- **改走 App 协议**。移动端走 `mtop.taobao.idlemtopsearch.search`，用的是 `x-sign` / `x-mini-wua` / `x-sgext` / `x-umt` 这一套**依赖真机 native 库**的签名族，与网页的 MD5 完全不是一回事；公开的复刻方案（unidbg 补环境）在 2026 年的最新实测结论仍是「能跑起来但服务端返回 `FAIL_SYS_ILLEGAL_SIGN`」。所以「手机上还能搜」**不等于**网页端能救回来。
- **反复重试**。这条最反直觉但最重要：`deny` 不接受任何人工操作（页面上就一句「访问被拒绝」），重试只会加深判定。

**浏览器窗口被手动关掉**也没关系：下一次搜索会自动重新拉起（否则会永久卡在 `Target page, context or browser has been closed`）。
...
src/
  ...
  search.mjs     搜索结果收集（browser 模式）：监听页面自身请求、多批合并、原生筛选、DOM 兜底
...
check-publish-dropdown.mjs  「新发布」下拉的离线验收：用实测 DOM 结构跑一遍，不碰闲鱼、不消耗风控额度
```
